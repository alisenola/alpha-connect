...
func (state *Listener) Initialize(context actor.Context) error {
	...
	state.executor = actor.NewPID(context.ActorSystem().Address(), "executor/exchanges/"+constants.UPBIT.Name+"_executor")
...
func (state *Listener) subscribeOrderbook(context actor.Context) error {
	...
	for nTries < 100 {
		...
		case upbit.WSOrderBook:
			...
			ts := uint64(obWs.Msg.ClientTime.UnixNano()) / 1000000
...
func (state *Listener) checkSockets(context actor.Context) error {
	if time.Since(state.lastOBPingTime) > 10*time.Second {
		// "Ping" by resubscribing to the topic
		_ = state.obWs.Ping()
		state.lastOBPingTime = time.Now()
	}

	if time.Since(state.lastTradePingTime) > 10*time.Second {
		// "Ping" by resubscribing to the topic
