...
func (state *Listener) readSocket(context actor.Context) error {
	...
	case msg := <-state.wsChan:
		...
		case bitfinex.WSSpotDepthL3Data:
			...
			/*
				if state.instruments[symbol].lastSequence + 1 != obData.Sequence {
					return fmt.Errorf("got inconsistent sequence number: %d, %d",
						state.instruments[symbol].lastSequence + 1, obData.Sequence)
				}
