...
func (state *Listener) Initialize(context actor.Context) error {
	...
	state.executor = actor.NewPID(context.ActorSystem().Address(), "executor/exchanges/"+constants.OKEX.Name+"_executor")
...
func (state *Listener) checkSockets(context actor.Context) error {
	if time.Since(state.lastPingTime) > 5*time.Second {
		...
		//_ = state.obWs.Subscribe(symbol, okex.WSBookL2TBTChannel)
		//_ = state.tradeWs.Subscribe(symbol, okex.WSTradesChannel)
		state.lastPingTime = time.Now()
	}
