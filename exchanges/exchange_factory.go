...
func NewAccount(accountInfo *models.Account, fillCollector *account.FillCollector, makerFees *float64) (*account.Account, error) {
	portfolioMx.Lock()
	defer portfolioMx.Unlock()
	...
	accnt, err := account.NewAccount(accountInfo, fillCollector, makerFees)
	if err != nil {
		return nil, err
	}
...
func NewInstrumentListenerProducer(securityID uint64, exchangeID uint32, dialerPool *utils.DialerPool, wsPool *utils.WebsocketPool) actor.Producer {
	...
	case constants.GATEF.ID:
		return func() actor.Actor { return gatef.NewListener(securityID, wsPool) }
