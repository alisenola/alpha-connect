...
func (state *Listener) subscribeInstrument(context actor.Context) error {
	...
	if err := ws.SubscribeMarketDepth(state.security.Symbol, huobip.WSOBLevel150, true); err != nil {
	...
	for nTries < 100 {
		...
		case huobip.WSMarketDepthTick:
			res := ws.Msg.Message.(huobip.WSMarketDepthTick)
		...
		case huobip.WSError:
			err := fmt.Errorf("error getting orderbook: %s", ws.Msg.Message.(huobip.WSError).ErrMsg)
			return err
		}
		nTries += 1
	}
...
func (state *Listener) onWebsocketMessage(context actor.Context) error {
	...
	case huobip.WSMarketDepthTick:
		...
		update := msg.Message.(huobip.WSMarketDepthTick)
	...
	case huobip.WSMarketTradeDetailTick:
		ts := uint64(msg.ClientTime.UnixNano() / 1000000)
		trades := msg.Message.(huobip.WSMarketTradeDetailTick)
